...
- 2026-04-18: **System Settings (Issue #12) — frontend scoping.** Inventoried ~40 hard-coded `'EUR'` currency pipes across 11 components and ~5 hard-coded `'dd/MM/yyyy'` date pipes across 4 components, all introduced after the spec was drafted. **Lesson:** when a new global service (settings, theme, locale) is being introduced, the cheapest way to prevent regressions is a centralised pipe (e.g. `core/pipes/date-format.pipe.ts`) that wraps Angular's pipe and reads from the service signal. One file, one mapping table, no drift.
- 2026-04-18: **Bootstrap discipline:** `APP_INITIALIZER` is the right pattern for any service whose value is read during initial render. Per-component constructor calls or route guards leak — a future feature page added by a teammate will forget the guard and reintroduce the flash-of-wrong-default. Always pair `APP_INITIALIZER` with a graceful fallback inside the service `load()` (try/catch + 1.5s timeout → defaults) so a slow Cosmos read can't stall app boot.
- 2026-04-18: `MatDateRangeInput` does not accept a date format parameter — the calendar UX is driven by Angular's locale, full stop. Don't try to override it; render a separate "selected range" preview label using the configured format if users need to see their setting reflected.
